...
app.layout = html.Div(
    style={
        "background-image": 'url("https://catherineasquithgallery.com/uploads/posts/2021-02/thumbs/1613682224_1-p-fon-dlya-prezentatsii-zheleznaya-doroga-1.jpg")',
    ...
    children=[
        html.Br(),
        html.H1(
            children="Координация пропуска вагонопотока",
        ...
        html.Div(
            [
                html.Div(
                    [
                        html.Label(
                            "Выберете экземпляр вагонопотока ",
                        ...
                        dcc.Dropdown(
                            hat, id="drop1", multi=False, style={"width": "500px"}
                        ),
                        html.Div(id="output1"),
                        html.Br(),
                        html.Button(
                            "Посмотреть вагонопоток",
                            id="button",
                            n_clicks=0,
                            style={
                                "fontSize": "14px",
...
@app.callback(Output("output1", "children"), Input("drop1", "value"))
...
def update_table(ecz, city, click):
    if click > 0:
        ...
        df = df.rename(
            columns={
                "trains": "Поезд",
                "number of cars": "Количество вагонов для сцепки",
